...
public class DirectoryClient {
	...
	*/   
	...
	public void call_attempt(final String usernames) {
		Log.i(TAG,"call attempt to " + usernames);
		writeHandler.post(new Runnable() {
			public void run() {
				try {
					synchronized(socket) {
						oos.writeObject(DirectoryCommand.C_CALL_ATTEMPT);//콜어템트 상태를 파일에 씀
						oos.writeObject(usernames);//유저네임을 파일에 쓰고
						oos.flush();//비움
	...
	public void call_hangup(final String usernames) {
