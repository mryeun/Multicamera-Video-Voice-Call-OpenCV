...
public class DirectoryServer {
	...
	private class Client {
					...
					while (!isInterrupted()) {
						try {
							...
							case C_CALL_REJECT:
								...
								break;
							case C_CALL_ATTEMPT:
								call_attempt((String) ois.readObject());// 전화걸기
								break;
							case C_CALL_ANSWER:
								call_answer((String) ois.readObject());// 전화 승낙
								break;
							case C_CALL_READY:
								call_ready();
								break;
							case C_CALL_HANGUP:
								call_hangup((String) ois.readObject());// 전화 끊기
								break;
							default:
		...
		// Method used for rejecting the call from the user : 전화 거부
		...
		private void call_attempt(String usernames) throws IOException {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				if (jsonArray.get(i).equals(username) == false) {
					Client clientThread2 = clientMap.get(jsonArray.get(i));// 클라이언트
					// 스레트를
					// 생성함
		...
		// accepted
		// 유저1이 유저2에게 전화를 걸었을 때 유저2가 전화를 승낙했을 때
